...
createApp({
	...
	methods: {
		...
		addContest() {
			...
			const newId = this.vueContests.length > 0 ? Math.max(...this.vueContests.map((t) => t.id)) + 1 : 1;
		...
		addJury() {
			...
			const newId = this.vueJuries.length > 0 ? Math.max(...this.vueJuries.map((t) => t.id)) + 1 : 1;
			currentJuries.push({
				id: newId,
				name: "",
				description: "",
		...
		addTeam() {
			...
			const newId = this.vueTeams.length > 0 ? Math.max(...this.vueTeams.map((t) => t.id)) + 1 : 1;
			currentTeams.push({
				id: newId,
				name: "",
				description: "",
